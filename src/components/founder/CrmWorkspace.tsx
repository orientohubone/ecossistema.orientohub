...
export const CrmWorkspace = ({ onCreateProposal }: { onCreateProposal: (client: CrmClient) => void }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([crmService.getNotes(selected.id), crmService.getTasks(selected.id)])
      .then(([clientNotes, clientTasks]) => { setNotes(clientNotes); setTasks(clientTasks); })
      .catch(() => setError('Não foi possível carregar a ficha completa do cliente.'))
  ...
  if (selected) {
    return (
      <div className="space-y-5" spellCheck={false}>
        ...
        <header className="rounded-2xl border border-[#273548] bg-[#101722] p-5 sm:p-6">
          <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
            ...
            <div className="flex flex-wrap gap-2">
              <select value={selected.stage} onChange={(event) => updateStage(selected, event.target.value as CrmStage)} className="rounded-xl border border-[#34455a] bg-[#0c121b] px-3 py-2.5 text-sm font-semibold text-white">{stages.map((stage) => <option key={stage.id} value={stage.id}>{stage.label}</option>)}</select>
        ...
        {error && <p className="rounded-xl border border-red-400/20 bg-red-400/5 p-3 text-sm text-red-200">{error}</p>}
          ...
          <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,.9fr)]">
            <div className="space-y-5">
              <section className="rounded-2xl border border-[#273548] bg-[#101722] p-5">
                ...
                <p className="mt-4 rounded-xl bg-[#151f2b] p-3 text-sm leading-relaxed text-[#9ba9bc]">{selected.demand || 'Sem demanda registrada.'}</p>
              </section>
  ...
  return (
    <div className="space-y-5" spellCheck={false}>
      ...
      {loading ? <div className="h-64 animate-pulse rounded-2xl bg-[#151f2b]" /> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">{stages.map((stage) => { const stageClients = filtered.filter((client) => client.stage === stage.id); return <section key={stage.id} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedClient) updateStage(draggedClient, stage.id); setDraggedClient(null); }} className={`min-h-[420px] rounded-2xl border p-2.5 ${draggedClient ? 'border-primary-400/45 bg-primary-500/5' : 'border-[#273548] bg-[#0c121b]/45'}`}><div className="mb-3 flex items-center justify-between px-1"><span className="text-xs font-bold uppercase tracking-[.12em] text-[#d7e0ea]">{stage.label}</span><span className={`rounded-md border px-2 py-0.5 text-[10px] font-bold ${stage.color}`}>{stageClients.length}</span></div><div className="space-y-2">{stageClients.map((client) => <button key={client.id} draggable onDragStart={() => setDraggedClient(client)} onDragEnd={() => setDraggedClient(null)} onClick={() => setSelected(client)} className={`w-full cursor-grab rounded-xl border p-3 text-left transition active:cursor-grabbing ${client.stage === 'novo' ? 'border-transparent bg-primary-500/15 hover:bg-primary-500/20' : 'border-[#273548] bg-[#101722] hover:border-[#41546b]'}`}><div className="flex items-center gap-2.5"><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${client.stage === 'novo' ? 'bg-primary-500 text-[#0c121b]' : 'bg-[#151f2b] text-primary-300'}`}>{client.company ? <Building2 className="h-4 w-4" /> : <UserRound className="h-4 w-4" />}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{client.name}</p><p className="truncate text-xs text-[#9ba9bc]">{client.company || client.email || 'Sem empresa informada'}</p></div><ChevronRight className="h-4 w-4 text-[#718096]" /></div><div className="mt-3 flex justify-between text-[11px] text-[#718096]"><span>{client.source === 'contato' ? 'Formulário' : 'Manual'}</span><span>{new Date(client.created_at).toLocaleDateString('pt-BR')}</span></div></button>)}{!stageClients.length && <div className="rounded-xl border border-dashed border-[#34455a] px-3 py-6 text-center text-xs text-[#718096]">Arraste uma oportunidade para cá</div>}</div></section>; })}</div>}
    </div>
  );
};
