...
import { Building2, ChevronRight, Mail, Plus, Search, Send, Trash2, UserRound, X } from 'lucide-react';
...
export const LeadsManagement = ({ onCreateProposal }: { onCreateProposal: (client: CrmClient) => void }) => {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', company: '', cnpj: '', address: '', services: [] as string[], demand: '' });
  ...
  const saveClient = async () => { if (!form.name.trim()) return; const created = await crmService.createClient(form); setClients((current) => [created, ...current]); setSelected(created); setForm({ name: '', email: '', phone: '', company: '', cnpj: '', address: '', services: [], demand: '' }); setNewClient(false); };
  const toggleService = (slug: string) => setForm((current) => ({ ...current, services: current.services.includes(slug) ? current.services.filter((service) => service !== slug) : [...current.services, slug] }));
