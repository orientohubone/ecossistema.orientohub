...
const EcosystemPage = () => {
  ...
  const [currentMVPIndex, setCurrentMVPIndex] = useState(0);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentMVPIndex((prev) => (prev + 1) % mvps.length);
    }, 5000); // Change slide every 5 seconds

    return () => clearInterval(interval);
  }, [isAutoScrolling, mvps.length]);

  const handlePrevMVP = () => {
    setIsAutoScrolling(false);
    setCurrentMVPIndex((prev) => (prev - 1 + mvps.length) % mvps.length);
  };
      ...
      <section id="mvps" className="py-24 bg-white dark:bg-gray-900">
        <div className="container-custom">
          ...
          <div className="relative max-w-5xl mx-auto">
            {/* Navigation Arrows */}
            <button
              onClick={handlePrevMVP}
              className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 lg:-translate-x-12 z-10 bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 p-3 rounded-full shadow-lg transition-all duration-300 hover:scale-110 border-2 border-gray-200 dark:border-gray-700"
            ...
            <button
              onClick={handleNextMVP}
              className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 lg:translate-x-12 z-10 bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 p-3 rounded-full shadow-lg transition-all duration-300 hover:scale-110 border-2 border-gray-200 dark:border-gray-700"
              aria-label="Próximo MVP"
            >
            ...
            <div className="flex justify-center gap-2 mt-8">
              {mvps.map((_, index) => (
                <button
                  ...
                  onClick={() => {
                    setIsAutoScrolling(false);
                    setCurrentMVPIndex(index);
                  }}
                  className={`h-2 rounded-full transition-all duration-300 ${index === currentMVPIndex
                      ? 'w-8 bg-primary-500'
                      : 'w-2 bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500'
                    }`}
                  aria-label={`Ir para MVP ${index + 1}`}
                />
              ))}
            </div>
            ...
            <div className="text-center mt-4">
              ...
              >
                {isAutoScrolling ? '⏸️ Pausar rotação automática' : '▶️ Retomar rotação automática'}
              </button>
            </div>
          </div>
